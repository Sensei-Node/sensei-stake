...
export function createOperatorDepositData(validatorKey, servicesContractAddress, network) {
  ...
  // added to check depositDataRoot signature
  const signatureDepositData = bls.sign(validatorKey.toBytes(), depositDataRoot);
  ...
  return {
    ...
    depositDataSignature: signatureDepositData,
    network
  };
}
...
export const verifySignatureDepositMessageRoot = (depositData) => {
  try {
    const signingRoot = SignatureSchema.hashTreeRoot({
      rootHash: depositData.depositMessageRoot,
      domain: generateDepositDomain(DomainType.DEPOSIT, GENESIS_VALIDATORS_ROOT, depositData.network)
    });
    return bls.verify(depositData.validatorPubKey, signingRoot, depositData.depositSignature);
  } catch (err) {
    console.error(err)
  }
};

export const verifySignatureGeneric = (pubkey, data, signature) => {
  try {
    return bls.verify(pubkey, data, signature);
  } catch (err) {
    console.error(err)
  }
};