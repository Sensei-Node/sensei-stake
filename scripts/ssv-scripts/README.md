...
- Bot: there is also a discord bot listening for these messages. After detecting a new one, it gets its validator public key using the `tokenId` and the `SenseiStake` smart contract. The bot then dispatches the validator registration process
